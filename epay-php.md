# Integration via PHP Library

> Below you can find detailed informations About Integration via PHP

# Requirements
1. PHP 7.2.5 and later. 
2. Get your ```API KEY```/```SECRET KEY``` from [ePay by Chargily](https://epay.chargily.com.dz)
...
```
2. Download Package in ZIP file

We do not recommend this option
But be careful to download the updated versions every a while [Download](https://github.com/Chargily/epay-gateway-php/releases/)

# Quick start

1. Firtly : Create config file ```epay_config.php```
...
```
2. Secondary : Create payment redirection file ```redirect.php```
...
```
3. Thirdly : create payment processing file ```process.php```
...
```
* Expected invoice statuses : ```paid``` , ```canceled``` and ```failed```
